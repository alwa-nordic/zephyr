...
/** Render LE address in canonical format
 *
 *  The returned string looks like "AB:CD:EF:01:02:03 (type)", with type
 *  replaced with one of public, public-id, random, and random-id.
 *
 *  The lifetime of the returned string ends at the end of the enclosing block.
 *
 *  Example usage:
 *      BT_INFO("My message to do with address %s.", bt_addr_le_str(addr));
 *
 *  Don't do this:
 *      char * addr_str;
 *      if (true) {
 *          addr_str = bt_addr_le_str(addr);
 *      }  // This the end of the enclosing block.
 *      // addr_str points to invalid memory here.
 */
#define bt_addr_le_str(addr) bt_addr_le_to_str_ret((addr), (char[BT_ADDR_LE_STR_LEN]) {})
static inline char *bt_addr_le_to_str_ret(const bt_addr_le_t *addr,
					  char str[static BT_ADDR_LE_STR_LEN])
{
	bt_addr_le_to_str(addr, str, BT_ADDR_LE_STR_LEN);
	return str;
}

/** Convert the argument to human readable string.
 *
 *  The lifetime of the returned string ends at the end of the enclosing block.
 *
 *  Example usage:
 *      BT_INFO("My message to do with address %s.", BT_STR(addr));
 */
#define BT_STR(x) _Generic((x), \
	const bt_addr_le_t * : bt_addr_le_str(x))
