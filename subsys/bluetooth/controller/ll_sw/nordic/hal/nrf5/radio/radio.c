...
#if defined(CONFIG_SOC_COMPATIBLE_NRF55F)
#define NRF_RADIO_HAS_ABSOLUTE_FREQ 1
#else
#define NRF_RADIO_HAS_ABSOLUTE_FREQ 0
#endif

#if defined(RADIO_SHORTS0_ResetValue) //TODO: Provisional HACK until the MDK comes with SHORTS0 named SHORTS again
#define SHORTS SHORTS0
#endif
...
#if (defined(CONFIG_SOC_COMPATIBLE_NRF54LX) || defined(CONFIG_SOC_COMPATIBLE_NRF55F))
#if defined(CONFIG_BT_CTLR_TIFS_HW)
...
#if !defined(CONFIG_SOC_SERIES_NRF51) && !(defined(CONFIG_SOC_COMPATIBLE_NRF54LX) || defined(CONFIG_SOC_COMPATIBLE_NRF55F))
...
#else
	NRF_RADIO->TXAMPLITUDESCALE = value;
#endif
...
void radio_pkt_configure(uint8_t bits_len, uint8_t max_len, uint8_t flags)
{
	...
	defined(CONFIG_SOC_COMPATIBLE_NRF54LX) || defined(CONFIG_SOC_COMPATIBLE_NRF55F)
	extra = 0U;
...
#if defined(CONFIG_BOARD_EULERBSIM) || defined(CONFIG_SOC_COMPATIBLE_NRF55F)
struct radio_vmda_job_ptr {
	void *ptr;
	struct {
		uint32_t length:24;
...
static struct radio_vmda_job_ptr vmda_job[2];
#endif
...
#else
	vmda_job[0].ptr = rx_packet;
	vmda_job[0].length = 256; //Just big enough
	vmda_job[0].attribute = 12;
	NRF_RADIO->VDMACONFIG.LISTPTR = (uint32_t)vmda_job;
	//TODO
#endif
}
...
#else
	vmda_job[1].ptr = tx_packet;
	vmda_job[1].length = 256; //Just big enough
	vmda_job[1].attribute = 12;

	NRF_RADIO->VDMACONFIG.LISTPTR = (uint32_t)&vmda_job[1];
	//TODO
#endif
}
...
{
#if !defined(CONFIG_BT_CTLR_TIFS_HW)
#if defined(CONFIG_SOC_COMPATIBLE_NRF5340_CPUNET) || (defined(CONFIG_SOC_COMPATIBLE_NRF54LX) || defined(CONFIG_SOC_COMPATIBLE_NRF55F))
...
#if (defined(CONFIG_SOC_COMPATIBLE_NRF54LX) || defined(CONFIG_SOC_COMPATIBLE_NRF55F)) && !defined(CONFIG_BOARD_EULERBSIM)
#if defined(CONFIG_NRF_SYS_EVENT)
