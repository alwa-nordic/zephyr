...
/**
 * Does the inverse of bt_conn_index() for ACL connections.
 * NB! index must carry a reference, which is moved into this function.
*/
struct bt_conn *bt_conn_from_acl_index(uint8_t acl_index)
{
	if (acl_index >= ARRAY_SIZE(acl_conns)) {
		return NULL;
	}

	/* We don't clone the reference here. We return the
	 * reference moved into this function.
	 */
	return &acl_conns[acl_index];
}
...
void host_flow_control_control_pool_free(struct net_buf *buf)
{
	net_buf_destroy(buf);
...
bool process_acl_data_ack(void)
{
	...
	for (uint8_t i = 0; i < ARRAY_SIZE(acl_conns); i++) {
		uint16_t ack_count;
