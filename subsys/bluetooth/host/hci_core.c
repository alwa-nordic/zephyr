...
static const struct event_handler meta_events_prio[] = {
	...
	EVENT_HANDLER(BT_HCI_EVT_LE_EXT_ADVERTISING_REPORT, bt_scan_append_ext_adv_report,
...
static void rx_work_handler(struct k_work *work)
{
	...
	if (!buf) {
		/* Scanner events are low priority. Only handle them when there
		 * are no other events.
		 */
		if (IS_ENABLED(CONFIG_BT_OBSERVER)) {
			bt_scan_rx_work();
	...
	if (!sys_slist_is_empty(&bt_dev.rx_queue) ||
	    (IS_ENABLED(BT_OBSERVER) && bt_scan_rx_work_pending())) {
		bt_hci_core_trigger_rx_work();
	}
}
