...
static void le_ltk_request(struct net_buf *buf)
{
	...
	if (IS_ENABLED(CONFIG_BT_HOOK_CONN_LTK_REQUEST)) {
		 ...
		 * the controller.
		 *
		 * This symbol is defined by the application. If you get a
		 * linker error here, please read the documentation for the
		 * hook.
		 *
		 * If the hook returns `true`, it cancels the default handler
		 */
