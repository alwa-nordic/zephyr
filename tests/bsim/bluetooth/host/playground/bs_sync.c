...
static void setup_backchannels(void)
{
	...
	uint self = bsim_args_get_global_device_nbr();
...
void bt_testlib_bs_sync_all(void)
{
	...
	LOG_DBG("%llu d%u enter", counter, bsim_args_get_global_device_nbr());
	...
	LOG_DBG("%llu d%u exit", counter, bsim_args_get_global_device_nbr());
