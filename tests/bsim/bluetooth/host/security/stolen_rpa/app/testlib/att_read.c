 ...
 */
int bt_testlib_gatt_discover_service(uint16_t *result_handle, uint16_t *result_end_handle,
				     struct bt_conn *conn, enum bt_att_chan_opt bearer,
				     enum bt_gatt_discover_type discover_type,
				     ...
				     uint16_t end_handle)
{
	...
	char uuid_str[BT_UUID_STR_SIZE];
	bt_uuid_to_str(uuid, uuid_str, sizeof(uuid_str));
	LOG_INF("Discovering conn %u, bearer %d, type %d, uuid %s, start_handle 0x%04x, "
		"end_handle 0x%04x",
		bt_conn_index(conn), bearer, discover_type, uuid_str, start_handle, end_handle);
...
int bt_testlib_gatt_discover_characteristic(uint16_t *const result_value_handle,
					    ...
					    enum bt_att_chan_opt bearer, const struct bt_uuid *uuid,
					    uint16_t start_handle, uint16_t svc_end_handle)
{
	int api_err;
	...
	char uuid_str[BT_UUID_STR_SIZE];
	bt_uuid_to_str(uuid, uuid_str, sizeof(uuid_str));
	LOG_INF("Discovering chrc conn %u, bearer %d uuid %s, start_handle 0x%04x, "
		"end_handle 0x%04x",
		bt_conn_index(conn), bearer, uuid_str, start_handle, svc_end_handle);
	api_err = bt_gatt_discover(conn, &ctx->params);
...
#if 0
int bt_testlib_find_by_type_value()
{
	uint8_t discover_type;
	if (type == BT_UUID_GATT_PRIMARY_VAL) {
		discover_type = BT_GATT_DISCOVER_PRIMARY;
	} else if (type == BT_UUID_GATT_SECONDARY_VAL) {
		discover_type = BT_GATT_DISCOVER_SECONDARY;
	} else {
