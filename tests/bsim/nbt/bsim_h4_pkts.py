...
def us_to_timestr(microseconds):
    """
    Render microseconds like '00:00:00.000000'.
    Almost like `str(datetime.timedelta(microseconds=x))`, but
    the microseconds are always rendered, even if zero.
    """
    assert microseconds >= 0
    seconds, microseconds = split_number(microseconds, 1000_000)
    minutes, seconds = split_number(seconds, 60)
    hours, minutes = split_number(minutes, 60)
    days, hours = split_number(hours, 24)
    assert days == 0
...
def main():
    ...
    for pkt in pkts:
            ...
            t = us_to_timestr(pkt_start_t_us)
            print(f"{direction} {t}")
            # The '000000' is the offset from start of packet of
            # the data that follows on this line. Packet data
            # can be split over multiple lines. '000000'
            # effectivly marks the start of a new packet.
